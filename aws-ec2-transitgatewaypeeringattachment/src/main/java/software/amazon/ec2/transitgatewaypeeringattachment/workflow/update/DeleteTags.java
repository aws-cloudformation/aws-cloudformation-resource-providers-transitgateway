...
public class DeleteTags {
    ...
    private List<Tag> tagsToDelete(ResourceModel model) {
        final List<software.amazon.ec2.transitgatewaypeeringattachment.Tag> prevTags =  new Read(this.proxy, this.request, this.callbackContext, this.client, this.logger).simpleRequest(model).getTags();
        final List<software.amazon.ec2.transitgatewaypeeringattachment.Tag> currTags = model.getTags();
