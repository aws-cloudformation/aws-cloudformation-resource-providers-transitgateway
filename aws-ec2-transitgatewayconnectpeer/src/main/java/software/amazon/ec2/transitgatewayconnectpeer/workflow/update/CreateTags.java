...
public class CreateTags {
    ...
    private List<Tag> tagsToCreate(ResourceModel model) {
        List<software.amazon.ec2.transitgatewayconnectpeer.Tag> prevTags = new Read(this.proxy, this.request, this.callbackContext, this.client, this.logger).simpleRequest(model).getTags();
        List<software.amazon.ec2.transitgatewayconnectpeer.Tag> currTags = TagUtils.mergeResourceModelAndStackTags(model.getTags(), this.request.getDesiredResourceTags());
