package software.amazon.ec2.transitgatewaymulticastgroup.workflow.read;
...
public class Read {
    ...
    private SearchTransitGatewayMulticastGroupsRequest translateModelToRequest(ResourceModel model) {
        ...
        filters.add(Filter.builder().name("is-group-member").values(model.getGroupMember().toString()).build());
        filters.add(Filter.builder().name("is-group-source").values(model.getGroupSource().toString()).build());
