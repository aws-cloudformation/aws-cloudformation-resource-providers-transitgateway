...
import software.amazon.ec2.transitgatewaymulticastdomain.Configuration;
...
public class ValidPropertiesCheck {
    ...
    public ProgressEvent<ResourceModel, CallbackContext>  run(ProgressEvent<ResourceModel, CallbackContext> progress) {
        if(this.request.getDesiredResourceState().getTransitGatewayMulticastDomainId() == null) {
    ...
    private List<String> invalidProperties() {
        ...
        return this.createOnlyProperties().stream().filter((prop) -> {
            try {
                ...
                logger.log(String.valueOf(previoudMethod));
