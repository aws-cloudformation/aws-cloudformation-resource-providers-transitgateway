...
public class DeleteTags {
    ...
    private List<Tag> tagsToDelete(ResourceModel model) {
        final List<com.aws.ec2.transitgatewaymulticastdomain.Tag> prevTags =  new Read(this.proxy, this.request, this.callbackContext, this.client, this.logger).simpleRequest(model).getTags();
        final List<com.aws.ec2.transitgatewaymulticastdomain.Tag> currTags = model.getTags();
