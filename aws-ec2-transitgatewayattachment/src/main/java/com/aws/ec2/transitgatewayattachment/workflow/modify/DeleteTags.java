...
public class DeleteTags {
    ...
    private List<Tag> tagsToDelete(ResourceModel model) {
        final List<com.aws.ec2.transitgatewayattachment.Tag> prevTags =  new Read(this.proxy, this.request, this.callbackContext, this.client, this.logger).simpleRequest(model).getTags();
        final List<com.aws.ec2.transitgatewayattachment.Tag> currTags = com.aws.ec2.transitgatewayattachment.workflow.TagUtils.mergeResourceModelAndStackTags(model.getTags(), this.request.getDesiredResourceTags());
        return TagUtils.difference(prevTags, currTags);
    }

    private ProgressEvent<ResourceModel, com.aws.ec2.transitgatewayattachment.CallbackContext>  handleError(DeleteTagsRequest awsRequest, Exception exception, ProxyClient<Ec2Client> client, ResourceModel model, CallbackContext context) {
