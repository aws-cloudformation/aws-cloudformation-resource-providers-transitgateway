package software.amazon.ec2.transitgatewayprefixlistreference.workflow;
...
public class ValidCurrentStateCheckBaseTest extends AbstractTestBase {
    ...
    public void failedForValidAvailableState() {
        ...
        when(proxyClient.client().getTransitGatewayPrefixListReferences(any(GetTransitGatewayPrefixListReferencesRequest.class))).thenReturn(MOCKS.describeResponse(TransitGatewayPrefixListReferenceState.DELETING.toString()));
        ResourceModel model = MOCKS.model();
        CallbackContext context =  new CallbackContext();
        ProgressEvent<ResourceModel, CallbackContext> response = new ValidAvailableState(proxy, MOCKS.request(model), context, proxyClient, logger).run(ProgressEvent.defaultInProgressHandler(context, 0, model));
        assertThat(response).isNotNull();
    ...
    public void throwError() {
        ...
        when(proxyClient.client().getTransitGatewayPrefixListReferences(any(GetTransitGatewayPrefixListReferencesRequest.class))).thenThrow((AwsServiceException.builder().awsErrorDetails(AwsErrorDetails.builder().errorMessage("NotFound").errorCode("NotFound").build())).build());
        ResourceModel model = MOCKS.model();
        CallbackContext context =  new CallbackContext();
        ProgressEvent<ResourceModel, CallbackContext> response = new ValidAvailableState(proxy, MOCKS.request(model), context, proxyClient, logger).run(ProgressEvent.defaultInProgressHandler(context, 0, model));
        assertThat(response).isNotNull();
