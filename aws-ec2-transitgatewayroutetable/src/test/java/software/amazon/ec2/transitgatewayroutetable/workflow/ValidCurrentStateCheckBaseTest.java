...
public class ValidCurrentStateCheckBaseTest extends AbstractTestBase {
    ...
    public void tear_down() {
        verify(sdkClient, atLeastOnce()).describeTransitGatewayRouteTables(any(DescribeTransitGatewayRouteTablesRequest.class));
    ...
    public void validForInvalidDeleteState() {
        ...
        when(proxyClient.client().describeTransitGatewayRouteTables(any(DescribeTransitGatewayRouteTablesRequest.class))).thenReturn(MOCKS.describeResponse(tags));
    ...
    public void throwError() {
        ...
        when(proxyClient.client().describeTransitGatewayRouteTables(any(DescribeTransitGatewayRouteTablesRequest.class))).thenThrow((AwsServiceException.builder().awsErrorDetails(AwsErrorDetails.builder().errorMessage("NotFound").errorCode("NotFound").build())).build());
        ResourceModel model = MOCKS.model(tags);
        CallbackContext context =  new CallbackContext();
        ProgressEvent<ResourceModel, CallbackContext> response = new ValidAvailableState(proxy, MOCKS.request(model), context, proxyClient, logger).run(ProgressEvent.defaultInProgressHandler(context, 0, model));
        assertThat(response).isNotNull();
