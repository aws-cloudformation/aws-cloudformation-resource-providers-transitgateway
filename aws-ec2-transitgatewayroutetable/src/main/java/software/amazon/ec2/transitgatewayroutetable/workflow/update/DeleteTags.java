...
public class DeleteTags {
    ...
    private DeleteTagsRequest translateModelToRequest(ResourceModel model) {
        ...
        return DeleteTagsRequest.builder()
            .resources(model.getTransitGatewayRouteTableId())
            .tags(tags)
            .build();
    }
    ...
    private List<Tag> tagsToDelete(ResourceModel model) {
        final List<software.amazon.ec2.transitgatewayroutetable.Tag> prevTags =  new Read(this.proxy, this.request, this.callbackContext, this.client, this.logger).simpleRequest(model).getTags();
        final List<software.amazon.ec2.transitgatewayroutetable.Tag> currTags = model.getTags();
